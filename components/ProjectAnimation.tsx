...
const ProjectAnimation: React.FC<ProjectAnimationProps> = ({ imageUrl }) => {
  ...
  useEffect(() => {
    ...
    const PALETTE = {
      LEMON: new THREE.Color('#E7FF6E'),
      PERIWINKLE: new THREE.Color('#6986E8'),
      PINK: new THREE.Color('#FFE5FF'),
      DARK: new THREE.Color('#1a1a1a')
    };
          ...
          for (let y = 0; y < img.height; y += step) {
            for (let x = 0; x < img.width; x += step) {
              ...
              if (a > 0.1) {
                ...
                let c;
                if (brightness < 0.2) c = PALETTE.DARK;
                else if (brightness < 0.45) c = PALETTE.PERIWINKLE;
                else if (brightness < 0.7) c = PALETTE.PINK;
                else c = PALETTE.LEMON;
  ...
  return (
    <div ref={containerRef} className="w-full bg-white overflow-hidden cursor-none aspect-video relative group">
       {!isLoaded && (
         <div className="absolute inset-0 flex items-center justify-center opacity-10 transition-opacity group-hover:opacity-20">
           <div className="text-[10px] uppercase tracking-[2em] font-mono">LOADING DATA</div>
         </div>
       )}
    </div>
  );
